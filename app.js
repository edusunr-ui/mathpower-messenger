...
const elements = {
  ...
  channelCreateForm: document.querySelector("#channel-create-form"),
  adminChannelName: document.querySelector("#admin-channel-name"),
  adminChannelDescription: document.querySelector("#admin-channel-description"),
  adminChannelType: document.querySelector("#admin-channel-type"),
  adminChannelSortOrder: document.querySelector("#admin-channel-sort-order"),
  adminMemberPicker: document.querySelector("#admin-member-picker"),
  channelSettingsForm: document.querySelector("#channel-settings-form"),
  adminSelectedChannelType: document.querySelector("#admin-selected-channel-type"),
  adminSelectedChannelSortOrder: document.querySelector("#admin-selected-channel-sort-order"),
  adminChannelMeta: document.querySelector("#admin-channel-meta"),
  channelMemberForm: document.querySelector("#channel-member-form"),
  adminMemberSelect: document.querySelector("#admin-member-select"),
  adminMemberRole: document.querySelector("#admin-member-role"),
  adminSelectedMemberList: document.querySelector("#admin-selected-member-list"),
  deleteChannelButton: document.querySelector("#delete-channel-button"),
  tabButtons: [...document.querySelectorAll(".tab-button")],
  tabPanels: {
    chat: document.querySelector("#tab-chat"),
    board: document.querySelector("#tab-board"),
    tasks: document.querySelector("#tab-tasks"),
    bookmarks: document.querySelector("#tab-bookmarks"),
  },
...
function bindEvents() {
  ...
  elements.channelCreateForm.addEventListener("submit", handleCreateChannel);
  elements.channelSettingsForm.addEventListener("submit", handleUpdateChannelSettings);
  elements.channelMemberForm.addEventListener("submit", handleAddMemberToChannel);
  elements.deleteChannelButton.addEventListener("click", handleDeleteChannel);
...
function setActiveTab(tab) {
  state.activeTab = Object.hasOwn(elements.tabPanels, tab) ? tab : "chat";
  elements.tabButtons.forEach((button) => {
...
async function hydrateWorkspace() {
  ...
  else state.allProfiles = [];
  await updatePresence("online");
  await Promise.all([loadUnreadCounts(), loadBookmarks()]);
  await loadChannels();
  setConnected(true);
  syncAuthPanel();
  renderProfile();
...
async function loadAllProfiles() {
  const { data, error } = await state.supabase
    .from("profiles")
    .select("id, name, role, homeroom")
    .order("name", { ascending: true });

  if (error) return notify("관리자용 프로필 목록을 불러오지 못했습니다.");
...
async function loadBookmarks() {
  ...
  const { data, error } = await state.supabase
    ...
    .select("id, message_id, channel_id, created_at, messenger_messages(id, sender_name, content, created_at, attachment_url), messenger_channels(name)")
    .eq("user_id", state.user.id)
    .order("created_at", { ascending: false });

  if (error) return notify("저장한 메시지를 불러오지 못했습니다.");
  state.bookmarks = data || [];
}

async function loadChannels() {
  const previouslySelectedId = state.selectedChannel?.id;
  const { data, error } = await state.supabase
    .from("messenger_channels")
    .select("id, name, description, type, member_count, sort_order")
  ...
  if (previouslySelectedId) {
    const refreshedSelection = state.channels.find((channel) => channel.id === previouslySelectedId);
    if (refreshedSelection) {
      state.selectedChannel = refreshedSelection;
      return;
    }
  }
...
async function loadMessages(channelId) {
  ...
  const bookmarkedIds = new Set(state.bookmarks.map((bookmark) => bookmark.message_id));
  state.messages = messages.map((message) => ({
    ...message,
    read_count: readCounts.get(message.id) || 0,
    is_read_by_me: myReadSet.has(message.id),
    is_bookmarked: bookmarkedIds.has(message.id),
  }));
...
async function toggleBookmark(message) {
  if (!state.user) return notify("로그인 후 저장 기능을 사용할 수 있습니다.");

  const existing = state.bookmarks.find((bookmark) => bookmark.message_id === message.id);
  if (existing) {
    ...
    if (error) return notify("저장 해제에 실패했습니다.");
  } else {
    const { error } = await state.supabase
      .from("messenger_bookmarks")
      .insert({
    ...
    if (error) return notify("메시지 저장에 실패했습니다.");
  }

  await Promise.all([loadBookmarks(), state.selectedChannel ? loadMessages(state.selectedChannel.id) : Promise.resolve()]);
  renderBookmarks();
}

async function handleCreateChannel(event) {
  event.preventDefault();
  if (!isAdmin()) return notify("관리자만 채널을 만들 수 있습니다.");

  const name = elements.adminChannelName.value.trim();
  const description = elements.adminChannelDescription.value.trim();
  const type = elements.adminChannelType.value;
  const sortOrder = Number(elements.adminChannelSortOrder.value || 0);
  const selectedIds = [...elements.adminMemberPicker.querySelectorAll('input[type="checkbox"]:checked')].map((input) => input.value);

  if (!name) return notify("채널 이름을 입력해주세요.");

  const memberIds = [...new Set([state.user.id, ...selectedIds])];
  ...
  const memberRows = memberIds.map((userId) => {
    const profile = state.allProfiles.find((entry) => entry.id === userId);
    return {
      channel_id: channel.id,
      user_id: userId,
      user_name: profile?.name || state.profile.name,
      role: userId === state.user.id ? "manager" : "member",
    };
  });
  ...
  if (memberError) return notify(`채널은 생성됐지만 멤버 배정에 실패했습니다. ${memberError.message}`);

  await updateChannelMemberCount(channel.id);
  elements.channelCreateForm.reset();
  renderAdminPanel();
  await loadChannels();
  const refreshed = state.channels.find((entry) => entry.id === channel.id) || channel;
  await selectChannel(refreshed);
  notify("새 채널을 만들었습니다.");
}

async function handleUpdateChannelSettings(event) {
  event.preventDefault();
  if (!isAdmin()) return notify("관리자만 채널 설정을 변경할 수 있습니다.");
  if (!state.selectedChannel) return notify("먼저 채널을 선택해주세요.");

  const type = elements.adminSelectedChannelType.value;
  const sortOrder = Number(elements.adminSelectedChannelSortOrder.value || 0);

  const { error } = await state.supabase
    .from("messenger_channels")
    .update({ type, sort_order: sortOrder })
    .eq("id", state.selectedChannel.id);

  if (error) return notify(error.message);

  await loadChannels();
  const refreshed = state.channels.find((channel) => channel.id === state.selectedChannel.id);
  if (refreshed) state.selectedChannel = refreshed;
  renderChannels();
  renderSelectedChannel();
  renderAdminPanel();
...
async function handleAddMemberToChannel(event) {
  event.preventDefault();
  if (!isAdmin()) return notify("관리자만 멤버를 배정할 수 있습니다.");
  if (!state.selectedChannel) return notify("먼저 채널을 선택해주세요.");

  const userId = elements.adminMemberSelect.value;
  const role = elements.adminMemberRole.value;
  if (!userId) return notify("추가할 멤버를 선택해주세요.");

  const profile = state.allProfiles.find((entry) => entry.id === userId);
  if (!profile) return notify("선택한 멤버 정보를 찾지 못했습니다.");
  ...
  await updateChannelMemberCount(state.selectedChannel.id);
  elements.channelMemberForm.reset();
  await Promise.all([loadChannels(), loadMembers(state.selectedChannel.id)]);
  renderChannels();
  renderAdminPanel();
  notify("멤버를 채널에 추가했습니다.");
}

async function handleUpdateMemberRole(userId, role) {
  if (!isAdmin()) return notify("관리자만 멤버 역할을 변경할 수 있습니다.");
  if (!state.selectedChannel) return notify("먼저 채널을 선택해주세요.");
  if (userId === state.user.id) return notify("현재 로그인한 자신의 채널 역할은 여기서 변경하지 않도록 막아두었습니다.");

  const target = state.members.find((member) => member.user_id === userId);
  if (!target) return notify("역할을 변경할 멤버를 찾지 못했습니다.");

  const { error } = await state.supabase
    .from("messenger_channel_members")
    .update({ role })
    .eq("channel_id", state.selectedChannel.id)
    .eq("user_id", userId);

  if (error) return notify(error.message);

  await loadMembers(state.selectedChannel.id);
  renderAdminPanel();
  notify(`${target.user_name}님의 채널 역할을 ${role}(으)로 변경했습니다.`);
}

async function handleDeleteChannel() {
  if (!isAdmin()) return notify("관리자만 채널을 삭제할 수 있습니다.");
  if (!state.selectedChannel) return notify("먼저 채널을 선택해주세요.");

  const channel = state.selectedChannel;
  const messageCount = state.messages.length;
  const memberCount = state.members.length || channel.member_count || 0;
  const warningMessage = [
    `'${channel.name}' 채널을 삭제할까요?`,
    "",
    `삭제 대상 메시지: ${messageCount}개`,
    `삭제 대상 멤버 배정: ${memberCount}명`,
    "",
    "이 작업은 되돌릴 수 없습니다.",
  ].join("\n");
...
async function handleRemoveMemberFromChannel(userId) {
  if (!isAdmin()) return notify("관리자만 멤버를 제거할 수 있습니다.");
  if (!state.selectedChannel) return notify("먼저 채널을 선택해주세요.");

  const target = state.members.find((member) => member.user_id === userId);
  if (!target) return notify("제거할 멤버를 찾지 못했습니다.");
  ...
  renderChannels();
  renderAdminPanel();
  notify(`${target.user_name}님을 채널에서 제거했습니다.`);
}

async function updateChannelMemberCount(channelId) {
...
function renderMessages() {
  ...
  filtered.forEach((message) => {
    ...
    bookmarkButton.textContent = message.is_bookmarked ? "저장됨" : "저장";
    bookmarkButton.classList.toggle("button--active", Boolean(message.is_bookmarked));
    ...
    fragment.querySelector(".reply-button").addEventListener("click", (event) => {
      event.stopPropagation();
      focusThread();
    });
...
function renderBookmarks() {
  ...
  if (!state.bookmarks.length) {
    elements.bookmarkList.innerHTML = '<p class="admin-empty">아직 저장한 메시지가 없습니다. 채팅 탭에서 메시지를 저장해보세요.</p>';
    return;
  }

  state.bookmarks.forEach((bookmark) => {
    const message = Array.isArray(bookmark.messenger_messages) ? bookmark.messenger_messages[0] : bookmark.messenger_messages;
    const channel = Array.isArray(bookmark.messenger_channels) ? bookmark.messenger_channels[0] : bookmark.messenger_channels;
    if (!message) return;
    ...
    card.innerHTML = `
      <div class="bookmark-card__top">
        <strong>${escapeHtml(message.sender_name || "알 수 없음")}</strong>
        <span class="bookmark-card__meta">저장일 ${formatDate(bookmark.created_at)}</span>
      </div>
      <div class="bookmark-card__meta">
        <span># ${escapeHtml(channel?.name || "채널")}</span>
        <span>${formatDate(message.created_at)}</span>
      </div>
      <p class="bookmark-card__content">${renderMentions(message.content || "(내용 없음)")}</p>
      <div class="bookmark-card__actions">
        <button type="button" class="button button--tiny">저장 해제</button>
      </div>
    `;

    const removeButton = card.querySelector("button");
    removeButton.addEventListener("click", async () => {
      await toggleBookmark({ id: bookmark.message_id, channel_id: bookmark.channel_id });
    });
...
function renderAdminPanel() {
  const adminEnabled = Boolean(state.user && isAdmin());
  elements.adminPanel.classList.toggle("hidden", !adminEnabled);
  if (!adminEnabled) return;

  renderAdminMemberPicker();
  renderAdminSelectedChannel();
}

function renderAdminMemberPicker() {
  elements.adminMemberPicker.innerHTML = "";
  if (!state.allProfiles.length) {
    elements.adminMemberPicker.innerHTML = '<p class="admin-empty">추가할 수 있는 프로필이 없습니다.</p>';
    return;
  }

  state.allProfiles.forEach((profile) => {
    const label = document.createElement("label");
    label.className = "member-picker__item";
    label.innerHTML = `
      <span>
        <strong>${escapeHtml(profile.name)}</strong>
        <span class="member-picker__meta">${escapeHtml(profile.role || "member")} · ${escapeHtml(profile.homeroom || "소속 미지정")}</span>
      </span>
      <input type="checkbox" value="${profile.id}" ${profile.id === state.user.id ? "checked" : ""} />
    `;
    const input = label.querySelector("input");
    if (profile.id === state.user.id) input.disabled = true;
    elements.adminMemberPicker.appendChild(label);
  });
}

function renderAdminSelectedChannel() {
  const hasChannel = Boolean(state.selectedChannel);
  elements.channelSettingsForm.querySelectorAll("input, select, button").forEach((element) => {
    element.disabled = !hasChannel;
  });
  elements.deleteChannelButton.disabled = !hasChannel;
  elements.channelMemberForm.querySelectorAll("select, button").forEach((element) => {
    element.disabled = !hasChannel;
  });

  if (!hasChannel) {
    elements.adminChannelMeta.textContent = "채널을 선택하면 설정과 멤버 관리가 열립니다.";
    elements.adminSelectedMemberList.innerHTML = '<p class="admin-empty">선택된 채널이 없습니다.</p>';
    elements.adminMemberSelect.innerHTML = '<option value="">추가할 멤버 선택</option>';
    return;
  }

  elements.adminChannelMeta.textContent = `${state.selectedChannel.name} · 현재 ${state.members.length}명`;
  elements.adminSelectedChannelType.value = state.selectedChannel.type || "channel";
  elements.adminSelectedChannelSortOrder.value = state.selectedChannel.sort_order ?? 0;

  const memberIds = new Set(state.members.map((member) => member.user_id));
  const addableProfiles = state.allProfiles.filter((profile) => !memberIds.has(profile.id));
  elements.adminMemberSelect.innerHTML = '<option value="">추가할 멤버 선택</option>';
  addableProfiles.forEach((profile) => {
    const option = document.createElement("option");
    option.value = profile.id;
    option.textContent = `${profile.name} · ${profile.role || "member"}`;
    elements.adminMemberSelect.appendChild(option);
  });

  elements.adminSelectedMemberList.innerHTML = "";
  if (!state.members.length) {
    elements.adminSelectedMemberList.innerHTML = '<p class="admin-empty">배정된 멤버가 없습니다.</p>';
    return;
  }
  ...
  state.members.forEach((member) => {
    ...
    card.innerHTML = `
      <div>
        <strong>${escapeHtml(member.user_name)}</strong>
        <p class="admin-member-card__meta">${escapeHtml(member.role || "member")}</p>
      </div>
      <div class="admin-member-card__actions"></div>
    `;

    const actions = card.querySelector(".admin-member-card__actions");
    const status = document.createElement("span");
    status.className = "member-picker__meta";
    status.textContent = member.user_id === state.user.id ? "현재 로그인 사용자" : "채널 배정됨";
    actions.appendChild(status);

    const roleSelect = document.createElement("select");
    roleSelect.className = "admin-member-role";
    ["member", "manager"].forEach((role) => {
      const option = document.createElement("option");
      option.value = role;
    ...
    roleSelect.disabled = member.user_id === state.user.id;
    actions.appendChild(roleSelect);

    if (member.user_id !== state.user.id) {
      const saveButton = document.createElement("button");
      saveButton.type = "button";
      saveButton.className = "button button--tiny";
      saveButton.textContent = "역할 저장";
      saveButton.addEventListener("click", () => handleUpdateMemberRole(member.user_id, roleSelect.value));
      ...
      button.addEventListener("click", () => handleRemoveMemberFromChannel(member.user_id));
      actions.appendChild(button);
    }

    elements.adminSelectedMemberList.appendChild(card);
  });
}
